...
def write_instance_to_example_file(
    ...
    for inst_index, instance in enumerate(tqdm(instances)):
        ...
        while len(input_ids) < max_seq_length:
            input_ids.append(pad_token_ids)
            input_mask.append(0)
        ...
        if i == len(document) - 1 or current_length >= target_seq_length:
            if current_chunk:
                ...
                for j in range(a_end):
                    tokens_a.extend(tokenizer.tokenize(current_chunk[j]))
...
def main():
    ...
    tokenizer = AutoTokenizer.from_pretrained(args.bert_model, use_fast= False
                                              , do_lower_case=args.do_lower_case, max_len=512)
